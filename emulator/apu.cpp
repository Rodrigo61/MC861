...
static FILE* fp;
void apu_init(){
    fp = popen("sox -e signed-integer -t s16 -c1 -r1789800 - -e signed-integer -t s16 -c2 - rate 48000 | aplay -fdat 2>/dev/null", "we");
}
...
int tick(int c)
{
    ...
    if(!channel_is_enabled[c]) return 8;
    ...
    switch(c)
    {
        ...
        case 1:// Square wave. With four different 8-step binary waveforms (32 bits of data total).
            if(wl < 8) S = 8;
            else S = (0xF33C0C04u & (1u << (++ch.phase % 8 + ch.duty_cycle * 8))) ? volume : 0;
            break;
        default:
            break;
    }
...
struct { uint16_t lo, hi; } hz240counter = { 0,0 };
double x = 0;
void apu_clock() 
{  
    ...
    if((hz240counter.lo += 2) >= 14915)
    {
        ...
        for(uint8_t c=0; c<4; c++)
        {
            ...
            if(half_tick && channels[c].length_counter
            && !(c==2 ? channels[c].linear_counter_disable : channels[c].length_counter_disable))
